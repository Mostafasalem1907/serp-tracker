<x-app-layout>
    <x-slot name="title">تعديل: {{ $project->name }}</x-slot>

    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">تعديل المشروع: {{ $project->name }}</h1>
        ...
        <div class="bg-white rounded-xl shadow p-6">
            <form method="POST" action="{{ route('projects.update', $project) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="domain" value="{{ old('domain', $project->domain) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    ...
                    <select name="country_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" {{ old('country_id', $project->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name_ar }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="language" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                            <option value="ar" {{ old('language', $project->language) === 'ar' ? 'selected' : '' }}>عربي</option>
                            <option value="en" {{ old('language', $project->language) === 'en' ? 'selected' : '' }}>English</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="device" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                            <option value="desktop" {{ old('device', $project->device) === 'desktop' ? 'selected' : '' }}>Desktop</option>
                            <option value="mobile" {{ old('device', $project->device) === 'mobile' ? 'selected' : '' }}>Mobile</option>
                        </select>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $project->is_active) ? 'checked' : '' }} class="rounded">
                    <label for="is_active" class="text-sm text-gray-700">مشروع نشط</label>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">حفظ التعديلات</button>
                    <a href="{{ route('projects.show', $project) }}" class="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-200 transition">إلغاء</a>
                </div>
            </form>
        </div>
    </div>
